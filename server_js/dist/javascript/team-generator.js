...
class TeamGenerator extends _lit.LitElement {
  ...
  generateAndDisplayTeams() {
    const {
      sets,
      pairs
    } = this.generateTeams(this.students, 4, 5);
    this.teams = sets;
    this.repeatedPairs = pairs;
    this.requestUpdate();
  }
  renderTeams() {
    return this.teams.map((set, index) => (0, _lit.html)`
                    <div class="team-set">
                        <h3>Team Set ${index + 1}</h3>
                        ${set.map((team, teamIndex) => (0, _lit.html)`
  ...
  renderReport() {
    return this.students.map(student => {
      const reportForStudent = this.students.filter(partner => student !== partner).map(partner => {
      ...
      return reportForStudent.length > 0 ? (0, _lit.html)`
